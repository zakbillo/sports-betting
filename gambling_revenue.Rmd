...
```
### cleaning the url scraped data so that it matches data types with the manually scarped data
...
total_state_revenue_raw = bind_rows(
  state_revenue_18 |> mutate(Year = 2018),
  state_revenue_19 |> mutate(Year = 2019),
  state_revenue_20 |> mutate(Year = 2020),
  state_revenue_21 |> mutate(Year = 2021),
  state_revenue_22 |> mutate(Year = 2022),
  state_revenue_23 |> mutate(Year = 2023),
...
monthly_grand_total = total_revenue_growth |> 
  summarize(
    ...
    Revenue = sum(Revenue, na.rm = TRUE),
    Taxes = sum(Taxes, na.rm = TRUE),
    Hold = sum(Revenue, na.rm = TRUE) / sum(Handle, na.rm = TRUE) # Recalculate true Hold %
  )
