...
plot_ly(
  ...
  type = "bar", 
  color = ~Year,
  text = ~scales::dollar(Total_Revenue / 1e9, scale = 1, suffix = "B"),
...
ordered_states = as.character(population_plot$State)
...
ordered_states = as.character(revenue_plot$State)
...
revenue_plot = 
  ...
  layout(
    ...
    xaxis = list(
      title = "State (sorted by population)",
      tickangle = 90,        
      tickmode = "array",           
      tickvals = ordered_states,  
      ticktext = ordered_states
...
combined_revenue_population = 
  ...
  layout(
    title = "Population vs Revenue Comparison by State",
    showlegend = FALSE
  )
...
race_plot |> 
  plot_ly(
    ...
    type = 'scatter',
    mode = 'lines+markers',
    text = ~paste(State, ": ", round(percentage, 1), "%"),
    hoverinfo = 'text',         
    line = list(width = 2)
  ) |> 
  layout(
