...
knitr::opts_chunk$set(
  fig.width = 10,  # Increased width for better fit
  fig.asp = 0.6,
  out.width = "95%",  # Increased from 90%
  fig.align = "center",
  message = FALSE,
  warning = FALSE,
  echo = TRUE
...
<style type="text/css"> /* Add these CSS rules to prevent graph overflow */ .plotly.html-widget { width: 100% !important; max-width: 100% !important; overflow: hidden !important; } .main-container { max-width: 1400px !important; margin: 0 auto !important; padding: 20px !important; } /* Ensure graphs don't overflow their containers */ .html-widget { margin: 10px 0 !important; padding: 10px 0 !important; } /* Better spacing between sections */ .section.level2 { margin-top: 40px !important; margin-bottom: 20px !important; } /* Fix for floating TOC */ .tocify { position: fixed !important; max-height: 80vh !important; overflow-y: auto !important; } body { padding-right: 300px !important; /* Space for TOC */ } @media (max-width: 1200px) { body { padding-right: 20px !important; } .tocify { display: none !important; } } </style>
...
plot_ly(data = plot_data) |> 
  ...
  layout(
    ...
    margin = list(l = 80, r = 80, b = 50, t = 50, pad = 4),
    autosize = TRUE,
    height = 450
  )
```
...
Below are exploratory plots of the demographic data in 2024. The first plot shows the per capita revenue for each state. The second plot shows gambling revenue by state in 2024.The third plot shows the racial makeup in percentage of the total population in each state in 2024, arranged in descending order of total gambling revenue. The fourth plot depicts the percentage of the total population in poverty and unemployed in 2024 in each state, arranged in descending order of total gambling revenue.
...
pred_illegal_lasso <- newdata_illegal |>
  mutate(
    ...
    predicted_revenue_lasso = pmax(predicted_revenue_lasso, 0) 
  ) |>
...
pred_illegal_lasso_plot <- pred_illegal_lasso |>
  mutate(State = reorder(State, predicted_revenue_lasso))
