...
population_plot = 
  population_plot_data |> 
  mutate(pop_millions = total_pop / 1e6) |> 
  plot_ly(
    y = ~State,
    x = ~pop_millions, 
    type = "bar", 
    color = ~State,
    text = ~paste0(round(pop_millions, 2), "M"),
    textposition = "outside",
    hovertemplate = paste0(
      "<b>State:</b> %{x}<br>",
      "<b>Population:</b> %{y:.2f}M<br>",
      "<extra></extra>"
    )
    ) |> 
  layout(
    title = "Total Population Size by State (2024)",
    yaxis = list(
    ...
    xaxis = list(
      title = "Total Population Size (Millions)",
      ticksuffix = "M"
    ),
    showlegend = FALSE,
    width = 900,
...
race_plot_data |> 
  plot_ly(
    x = ~State,
    y = ~percentage * 100,  # Convert to percentage
    color = ~race,
    type = 'bar',
    hovertemplate = paste0(
      "<b>State:</b> %{x}<br>",
